...
def to_json(plan: ProductPlan) -> str:
    ...
    payload: dict[str, Any] = {
        ...
        "totals": {
            ...
            "asynchronous": len(plan.asynchronous),
        ...
        "operations": [
            {
                ...
                "asynchronous": item.operation.is_async,
...
def to_markdown(plan: ProductPlan) -> str:
    ...
    lines: list[str] = [
        ...
        f"Dont asynchrones (réponse `operation`) : **{len(plan.asynchronous)}**",
        "",
        "## Classification",
        "",
        "| classe | opérations |",
    ...
    lines += [
        ...
        "## L'asynchrone",
        "",
        f"{len(plan.asynchronous)} opérations sur {len(plan.operations)} répondent par",
        "un objet `operation` : l'API accepte le travail et rend son identifiant.",
        "Un module qui rendrait `changed` à cet instant mentirait. Le runtime",
        "interroge l'opération jusqu'à `success`, `failure` ou `timeout` quand",
        "`wait` est vrai, et le dit dans son retour.",
        "",
        "## Modules cibles",
        "",
        "| module | opérations |",
...
def to_text(plan: ProductPlan) -> str:
    ...
    lines = [
        ...
        f"Asynchronous (operation object): {len(plan.asynchronous)}",
        "",
    ]
    for kind in _KIND_ORDER:
        if counts[kind]:
    ...
    for item in plan.operations:
        ...
        flag = "async" if item.operation.is_async else ""
        lines.append(
            f"{item.operation.id:<46} {item.operation.http_method.value:<7} "
            f"{item.resource:<28} {item.kind.value.upper():<10} "
            f"{item.mode.value.upper():<9} {module:<40} {flag}".rstrip()
        )
