...
# Opérations : reset-dbaas-valkey-user-password
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_valkey_password_action
short_description: Perform an action on an Exoscale dbaas valkey password
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas valkey password: C(reset_dbaas_valkey_user_password).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas valkey password.
    type: str
    required: true
    choices:
...
EXAMPLES = r"""
- name: Run reset_dbaas_valkey_user_password on a dbaas valkey password
  stephrobert.exoscale.dbaas_valkey_password_action:
    zone: ch-gva-2
    action: reset_dbaas_valkey_user_password
...
MODULE_ARGUMENT_SPEC = {
    "action": {
        "type": "str",
        "required": True,
        "choices": ["reset_dbaas_valkey_user_password"],
    },
...
MODULE = ActionModule(
    resource="dbaas_valkey_password",
    selector=None,
    actions=(
        Action(
            name="reset_dbaas_valkey_user_password",
            operation=Operation(
                id="reset-dbaas-valkey-user-password",
                method="reset_dbaas_valkey_user_password",
