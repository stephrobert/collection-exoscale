...
# Opérations : stop-dbaas-valkey-migration
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_valkey_migration_action
short_description: Perform an action on an Exoscale dbaas valkey migration
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas valkey migration: C(stop).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas valkey migration.
    type: str
    required: true
    choices:
    - stop
...
EXAMPLES = r"""
- name: Run stop on a dbaas valkey migration
  stephrobert.exoscale.dbaas_valkey_migration_action:
    zone: ch-gva-2
    action: stop
...
MODULE = ActionModule(
    resource="dbaas_valkey_migration",
    selector="name",
    actions=(
        Action(
            name="stop",
            operation=Operation(
                id="stop-dbaas-valkey-migration",
                method="stop_dbaas_valkey_migration",
