...
# Opérations : start-dbaas-clickhouse-maintenance
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_clickhouse_maintenance_action
short_description: Perform an action on an Exoscale dbaas clickhouse maintenance
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas clickhouse maintenance: C(start).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas clickhouse maintenance.
    type: str
    required: true
    choices:
    - start
...
EXAMPLES = r"""
- name: Run start on a dbaas clickhouse maintenance
  stephrobert.exoscale.dbaas_clickhouse_maintenance_action:
    zone: ch-gva-2
    action: start
...
MODULE = ActionModule(
    resource="dbaas_clickhouse_maintenance",
    selector="name",
    actions=(
        Action(
            name="start",
            operation=Operation(
                id="start-dbaas-clickhouse-maintenance",
                method="start_dbaas_clickhouse_maintenance",
