...
# Opérations : reset-dbaas-clickhouse-user-password
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_clickhouse_password_action
short_description: Perform an action on an Exoscale dbaas clickhouse password
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas clickhouse password: C(reset_dbaas_clickhouse_user_password).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas clickhouse password.
    type: str
    required: true
    choices:
...
EXAMPLES = r"""
- name: Run reset_dbaas_clickhouse_user_password on a dbaas clickhouse password
  stephrobert.exoscale.dbaas_clickhouse_password_action:
    zone: ch-gva-2
    action: reset_dbaas_clickhouse_user_password
...
MODULE_ARGUMENT_SPEC = {
    "action": {
        "type": "str",
        "required": True,
        "choices": ["reset_dbaas_clickhouse_user_password"],
    },
...
MODULE = ActionModule(
    resource="dbaas_clickhouse_password",
    selector=None,
    actions=(
        Action(
            name="reset_dbaas_clickhouse_user_password",
            operation=Operation(
                id="reset-dbaas-clickhouse-user-password",
                method="reset_dbaas_clickhouse_user_password",
