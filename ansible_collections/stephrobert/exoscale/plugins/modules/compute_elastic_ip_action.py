...
# Opérations : reset-elastic-ip-field
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: compute_elastic_ip_action
short_description: Perform an action on an Exoscale elastic ip
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing elastic ip: C(reset_field).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the elastic ip.
    type: str
    required: true
    choices:
    - reset_field
...
EXAMPLES = r"""
- name: Run reset_field on a elastic ip
  stephrobert.exoscale.compute_elastic_ip_action:
    zone: ch-gva-2
    action: reset_field
