...
# Opérations : stop-dbaas-pg-migration
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_postgres_migration_action
short_description: Perform an action on an Exoscale dbaas postgres migration
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas postgres migration: C(stop_dbaas_pg_migration).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas postgres migration.
    type: str
    required: true
    choices:
...
EXAMPLES = r"""
- name: Run stop_dbaas_pg_migration on a dbaas postgres migration
  stephrobert.exoscale.dbaas_postgres_migration_action:
    zone: ch-gva-2
    action: stop_dbaas_pg_migration
...
MODULE_ARGUMENT_SPEC = {
    "action": {
        "type": "str",
        "required": True,
        "choices": ["stop_dbaas_pg_migration"],
    },
...
MODULE = ActionModule(
    resource="dbaas_postgres_migration",
    selector="name",
    actions=(
        Action(
            name="stop_dbaas_pg_migration",
            operation=Operation(
                id="stop-dbaas-pg-migration",
                method="stop_dbaas_pg_migration",
