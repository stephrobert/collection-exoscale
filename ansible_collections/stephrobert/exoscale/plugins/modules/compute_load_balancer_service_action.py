...
# Opérations : reset-load-balancer-service-field
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: compute_load_balancer_service_action
short_description: Perform an action on an Exoscale load balancer service
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing load balancer service: C(reset_field).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the load balancer service.
    type: str
    required: true
    choices:
    - reset_field
...
EXAMPLES = r"""
- name: Run reset_field on a load balancer service
  stephrobert.exoscale.compute_load_balancer_service_action:
    zone: ch-gva-2
    action: reset_field
...
MODULE = ActionModule(
    resource="load_balancer_service",
    selector=None,
    actions=(
        Action(
            name="reset_field",
            operation=Operation(
                ...
                method="reset_load_balancer_service_field",
                path_params={"id": "id", "service_id": "service-id", "field": "field"},
                is_async=True,
            ),
        ),
    ),
