...
# Opérations : export-snapshot
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: compute_snapshot_action
short_description: Perform an action on an Exoscale snapshot
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing snapshot: C(export).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the snapshot.
    type: str
    required: true
    choices:
    - export
...
EXAMPLES = r"""
- name: Run export on a snapshot
  stephrobert.exoscale.compute_snapshot_action:
    zone: ch-gva-2
    action: export
