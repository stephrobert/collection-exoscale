...
# Opérations : get-dns-domain, list-dns-domains
...
short_description: Gather information about Exoscale dns domains
version_added: 0.1.0
description:
- Read one Exoscale dns domain by its identifier, or list them. This module never changes
  anything.
author:
...
- name: List dns domains
  stephrobert.exoscale.dns_domain_info:
    zone: ch-gva-2
  register: result
- name: Read one dns domain
  stephrobert.exoscale.dns_domain_info:
    zone: ch-gva-2
...
RETURN = r"""
dns_domain:
  description: The dns domain, when a selector is given.
  returned: when the selector is given
  type: dict
dns_domains:
  description: The dns domains.
...
MODULE = InfoModule(
    ...
    list_operation=Operation(
        id="list-dns-domains",
        method="list_dns_domains",
        path_params={},
        payload_field="dns-domains",
        is_list=True,
    ),
    selector="id",
)
