...
# Opérations : reset-dbaas-mysql-user-password
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_mysql_password_action
short_description: Perform an action on an Exoscale dbaas mysql password
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas mysql password: C(reset_dbaas_mysql_user_password).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas mysql password.
    type: str
    required: true
    choices:
...
EXAMPLES = r"""
- name: Run reset_dbaas_mysql_user_password on a dbaas mysql password
  stephrobert.exoscale.dbaas_mysql_password_action:
    zone: ch-gva-2
    action: reset_dbaas_mysql_user_password
...
MODULE_ARGUMENT_SPEC = {
    "action": {
        "type": "str",
        "required": True,
        "choices": ["reset_dbaas_mysql_user_password"],
    },
    ...
    "authentication": {
        "type": "str",
        "choices": ["caching_sha2_password", "mysql_native_password"],
    },
}
...
MODULE = ActionModule(
    resource="dbaas_mysql_password",
    selector=None,
    actions=(
        Action(
            name="reset_dbaas_mysql_user_password",
            operation=Operation(
                id="reset-dbaas-mysql-user-password",
                method="reset_dbaas_mysql_user_password",
                path_params={"service_name": "service-name", "username": "username"},
                body_params={"password": "password", "authentication": "authentication"},
                is_async=True,
            ),
        ),
    ),
