...
# Opérations : enable-dbaas-mysql-writes
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_mysql_write_action
short_description: Perform an action on an Exoscale dbaas mysql write
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas mysql write: C(enable).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas mysql write.
    type: str
    required: true
    choices:
    - enable
...
EXAMPLES = r"""
- name: Run enable on a dbaas mysql write
  stephrobert.exoscale.dbaas_mysql_write_action:
    zone: ch-gva-2
    action: enable
...
MODULE = ActionModule(
    resource="dbaas_mysql_write",
    selector="name",
    actions=(
        Action(
            name="enable",
            operation=Operation(
                id="enable-dbaas-mysql-writes",
                method="enable_dbaas_mysql_writes",
