...
# Opérations : get-kms-key, list-kms-keys
...
DOCUMENTATION = r"""
module: kms_key_info
short_description: Gather information about Exoscale kms keys
version_added: 0.1.0
description:
- Read one Exoscale kms key by its identifier, or list them. This module never changes anything.
author:
...
EXAMPLES = r"""
- name: List kms keys
  stephrobert.exoscale.kms_key_info:
    zone: ch-gva-2
  register: result
- name: Read one kms key
  stephrobert.exoscale.kms_key_info:
    zone: ch-gva-2
...
RETURN = r"""
kms_key:
  description: The kms key, when a selector is given.
...
MODULE = InfoModule(
    ...
    list_operation=Operation(
        id="list-kms-keys",
        method="list_kms_keys",
        path_params={},
        payload_field="kms-keys",
        is_list=True,
    ),
    selector="id",
)
