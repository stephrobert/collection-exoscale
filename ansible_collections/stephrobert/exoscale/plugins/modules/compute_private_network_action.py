...
# Opérations : reset-private-network-field
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: compute_private_network_action
short_description: Perform an action on an Exoscale private network
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing private network: C(reset_field).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the private network.
    type: str
    required: true
    choices:
    - reset_field
...
EXAMPLES = r"""
- name: Run reset_field on a private network
  stephrobert.exoscale.compute_private_network_action:
    zone: ch-gva-2
    action: reset_field
    ...
    actions=(
        Action(
            ...
            operation=Operation(
                ...
                method="reset_private_network_field",
