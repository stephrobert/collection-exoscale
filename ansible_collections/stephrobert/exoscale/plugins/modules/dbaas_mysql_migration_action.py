...
# Opérations : stop-dbaas-mysql-migration
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_mysql_migration_action
short_description: Perform an action on an Exoscale dbaas mysql migration
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas mysql migration: C(stop).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas mysql migration.
    type: str
    required: true
    choices:
    - stop
...
EXAMPLES = r"""
- name: Run stop on a dbaas mysql migration
  stephrobert.exoscale.dbaas_mysql_migration_action:
    zone: ch-gva-2
    action: stop
...
MODULE = ActionModule(
    resource="dbaas_mysql_migration",
    selector="name",
    actions=(
        Action(
            name="stop",
            operation=Operation(
                ...
                method="stop_dbaas_mysql_migration",
