...
# Opérations : start-dbaas-kafka-maintenance
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_kafka_maintenance_action
short_description: Perform an action on an Exoscale dbaas kafka maintenance
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas kafka maintenance: C(start).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas kafka maintenance.
    type: str
    required: true
    choices:
    - start
...
EXAMPLES = r"""
- name: Run start on a dbaas kafka maintenance
  stephrobert.exoscale.dbaas_kafka_maintenance_action:
    zone: ch-gva-2
    action: start
...
MODULE = ActionModule(
    resource="dbaas_kafka_maintenance",
    selector="name",
    actions=(
        Action(
            name="start",
            operation=Operation(
                ...
                method="start_dbaas_kafka_maintenance",
