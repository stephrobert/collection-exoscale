...
# Opérations : start-dbaas-valkey-maintenance
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_valkey_maintenance_action
short_description: Perform an action on an Exoscale dbaas valkey maintenance
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas valkey maintenance: C(start).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas valkey maintenance.
    type: str
    required: true
    choices:
    - start
...
EXAMPLES = r"""
- name: Run start on a dbaas valkey maintenance
  stephrobert.exoscale.dbaas_valkey_maintenance_action:
    zone: ch-gva-2
    action: start
...
MODULE = ActionModule(
    resource="dbaas_valkey_maintenance",
    selector="name",
    actions=(
        Action(
            name="start",
            operation=Operation(
                id="start-dbaas-valkey-maintenance",
                method="start_dbaas_valkey_maintenance",
