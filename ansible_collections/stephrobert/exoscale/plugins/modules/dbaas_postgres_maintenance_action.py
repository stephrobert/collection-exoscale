...
# Opérations : start-dbaas-pg-maintenance
# Régénérer  : mise run generate

from __future__ import annotations

DOCUMENTATION = r"""
module: dbaas_postgres_maintenance_action
short_description: Perform an action on an Exoscale dbaas postgres maintenance
version_added: 0.1.0
description:
- 'Trigger one of the following actions on an existing dbaas postgres maintenance: C(start_dbaas_pg_maintenance).'
author:
- Stéphane Robert (@stephrobert)
options:
  action:
    description: The action to trigger on the dbaas postgres maintenance.
    type: str
    required: true
    choices:
...
EXAMPLES = r"""
- name: Run start_dbaas_pg_maintenance on a dbaas postgres maintenance
  stephrobert.exoscale.dbaas_postgres_maintenance_action:
    zone: ch-gva-2
    action: start_dbaas_pg_maintenance
...
MODULE_ARGUMENT_SPEC = {
    "action": {
        "type": "str",
        "required": True,
        "choices": ["start_dbaas_pg_maintenance"],
    },
...
MODULE = ActionModule(
    resource="dbaas_postgres_maintenance",
    selector="name",
    actions=(
        Action(
            name="start_dbaas_pg_maintenance",
            operation=Operation(
                id="start-dbaas-pg-maintenance",
                method="start_dbaas_pg_maintenance",
