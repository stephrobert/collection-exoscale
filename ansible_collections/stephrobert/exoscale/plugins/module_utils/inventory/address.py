...
#: gère un parc est le plus souvent dedans, et qu'une adresse privée ne dépend
#: pas de l'attribution d'une IP publique.
...
def select_ansible_host(host: InventoryHost, policy: AddressPolicy) -> AddressSelection:
    ...
    * **par réseau privé** : seules les adresses du réseau nommé sont
      ...
      jointe par `backend` de façon déterministe.
    ...
    if policy.private_network is not None:
        ...
        nom_reseau = attachement.private_network_name or attachement.private_network_id
        if trouve is None:
            return AddressSelection(
                address=None,
                source=f"aucune adresse sur le réseau '{policy.private_network}'",
                private_network=nom_reseau,
                considered=ordre,
            )
        adresse, famille = trouve
        return AddressSelection(
            address=adresse, source=famille, private_network=nom_reseau, considered=ordre
        )
