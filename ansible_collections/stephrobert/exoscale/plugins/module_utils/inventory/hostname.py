...
Deux machines Exoscale peuvent porter le même nom : le nom n'est unique ni
dans une zone ni dans l'organisation, et un pool d'instances nomme ses membres
d'après le sien. Un inventaire qui l'ignore appelle `add_host` deux fois avec
la même clé, et la seconde machine **écrase** la première, ses variables et
ses groupes. Sans un mot.
...
#: Préfixe qui lit un label par sa clé : `label:role` lit `labels["role"]`.
LABEL_PREFIX = "label:"
...
def is_known_source(source: str) -> bool:
    ...
    if source.startswith(LABEL_PREFIX):
        return bool(source[len(LABEL_PREFIX) :].strip())
...
def resolve_source(host: InventoryHost, source: str) -> str | None:
    ...
    if source.startswith(LABEL_PREFIX):
        cle = source[len(LABEL_PREFIX) :]
        valeur = str(host.labels.get(cle, "") or "").strip()
...
def assign_hostnames(
    ...
    Exoscale étant unique, la boucle se termine toujours.
    ...
    for host in hosts:
        ...
        for suffixe in (host.zone, host.id):
            ...
            if candidat not in pris:
                ...
                break
        else:  # pragma: no cover - un identifiant Exoscale est unique
